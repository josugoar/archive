...
#define ESP_BD_ADDR_STR_LEN 18
#define BLINK_PERIOD_SCALE 10

TickType_t blink_period = CONFIG_BLINK_PERIOD;
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_CONNECT_EVT:
    {
        ...
        esp_ble_gap_read_rssi(p_data->connect.remote_bda);
        break;
    }
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT:
    {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            char bda[ESP_BD_ADDR_STR_LEN];
            sprintf(bda, ESP_BD_ADDR_STR, ESP_BD_ADDR_HEX(scan_result->scan_rst.bda));
            if (strcmp(bda, CONFIG_BDA) != 0)
            {
                break;
            }
            ...
            adv_name = esp_ble_resolve_adv_data_by_type(scan_result->scan_rst.ble_adv,
                                                        ...
                                                        &adv_name_len);
            ESP_LOGI(GATTC_TAG, "Scan result, device " ESP_BD_ADDR_STR ", name len %u, rssi %d", ESP_BD_ADDR_HEX(scan_result->scan_rst.bda), adv_name_len, scan_result->scan_rst.rssi);
    ...
    case ESP_GAP_BLE_READ_RSSI_COMPLETE_EVT:
        if (param->scan_start_cmpl.status != ESP_BT_STATUS_SUCCESS)
        {
            ESP_LOGE(GATTC_TAG, "Read RSSI failed, status %x", param->scan_start_cmpl.status);
            break;
        }
        ESP_LOGI(GATTC_TAG, "RSSI: %d dBm", param->read_rssi_cmpl.rssi);
        blink_period = (param->read_rssi_cmpl.rssi + 127) * BLINK_PERIOD_SCALE;
...
void app_main(void)
{
    ...
    xTaskCreate(blink_task, "blink_task", 4096, NULL, 0, NULL);
}
