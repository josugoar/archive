...
#endif
{
    ...
    for (size_t i = 0; i < sizeof(remote_fds) / sizeof(*remote_fds); ++i)
    {
        remote_fds[i] = -1;
    }
    ...
    for (struct addrinfo *ai = ais; ai; ai = ai->ai_next)
    {
        ...
        if (getnameinfo(ai->ai_addr, ai->ai_addrlen, host, sizeof(host), serv, sizeof(serv), NI_NUMERICHOST | NI_NUMERICSERV) != 0)
        {
            fprintf(stderr, "W tcp_server: getnameinfo fd=%d: %s\n", local_fd, strerror(errno));
        }

        if (*host != '\0' && *serv != '\0')
        {
            fprintf(stderr, "I tcp_server: local_fd=%s host=%s serv=%s\n", local_fd, host, serv);
        }

        break;
    }

    if (listen(local_fd, sizeof(remote_fds) / sizeof(*remote_fds)) != 0)
    {
    ...
    while (true)
    {
        ...
        if (FD_ISSET(acceptfd, &curr_readfds) != 0)
        {
            while (true)
            {
                ...
                for (; i < sizeof(remote_fds) / sizeof(*remote_fds); ++i)
                {
                ...
                if (i >= sizeof(remote_fds) / sizeof(*remote_fds))
                {
                ...
                int readfd = accept(acceptfd, (struct addrinfo *)&addr, &addr_len);
                if (readfd == -1)
                {
                ...
                if (*host != '\0' && *serv != '\0')
                {
                    fprintf(stderr, "I tcp_server: remote_fd=%s host=%s serv=%s\n", remote_fd, host, serv);
                }
        ...
        for (size_t i = 0; i < sizeof(remote_fds) / sizeof(*remote_fds); ++i)
        {
            ...
            while (true)
            {
                ...
                if (n == 0)
                {
                    if (shutdown(readfd, SHUT_RDWR) != 0)
                    {
                        fprintf(stderr, "W tcp_server: shutdown fd=%d: %s\n", readfd, strerror(errno));
                    }
            ...
            {
                for (size_t j = 0; j < sizeof(remote_fds) / sizeof(*remote_fds); ++j)
                {
