package org.ccrew.cchess;

import static org.junit.Assert.assertEquals;
import static org.junit.Assert.assertFalse;
import static org.junit.Assert.assertTrue;

import org.ccrew.cchess.lib.ChessResult;
import org.ccrew.cchess.lib.ChessRule;
...
public class ChessGameTest {
    ...
    protected static void testGoodMove(String fen, String move, String resultFen, ChessResult result, ChessRule rule,
        ...
        assertTrue(String.format("%s + %s is an invalid move", fen, move), state.move(move));

        assertEquals(String.format("%s + %s has state %s not %s", fen, move, state.getFen(), resultFen), resultFen,
                state.getFen());
        ...
        // accepted.
        if (verifySAN) {
            assertEquals(String.format("%s + %s has SAN move %s", fen, move, state.lastMove.getSan()), move,
                    state.lastMove.getSan());
        }

        Out<ChessRule> moveRule = new Out<>();
        var moveResult = state.getResult(moveRule);
        assertEquals(String.format("%s + %s has result %s not %s", fen, move, moveResult, result), result, moveResult);
        assertEquals(String.format("%s + %s has result %s not %s", fen, move, moveResult, result), rule,
                moveRule.value);
    }

    protected static void testBadMove(String fen, String move) {
        ChessState state = new ChessState(fen);
        assertFalse(String.format("%s + %s is valid", fen, move), state.move(move, false));
    }
